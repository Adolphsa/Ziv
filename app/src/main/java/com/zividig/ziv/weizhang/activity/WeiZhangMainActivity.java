...
public class WeiZhangMainActivity extends Activity {
	...
	private boolean checkQueryItem(CarInfo car) {
		if (car.getCity_id() == 0) {
			Toast.makeText(WeiZhangMainActivity.this, "请选择查询地", Toast.LENGTH_SHORT).show();
			return false;
		}

		if (car.getChepai_no().length() != 7) {
			Toast.makeText(WeiZhangMainActivity.this, "您输入的车牌号有误", Toast.LENGTH_SHORT).show();
			return false;
		}
		...
		if (car.getCity_id() > 0) {
			...
			if (classno > 0) {
				if (car.getChejia_no().equals("")) {
					Toast.makeText(WeiZhangMainActivity.this, "输入车架号不为空", Toast.LENGTH_SHORT).show();
					return false;
				}

				if (car.getChejia_no().length() != classno) {
					Toast.makeText(WeiZhangMainActivity.this, "输入车架号后" + classno + "位",
							Toast.LENGTH_SHORT).show();
					return false;
				}
			} else if (classno < 0) {
				if (car.getChejia_no().length() == 0) {
					Toast.makeText(WeiZhangMainActivity.this, "输入全部车架号", Toast.LENGTH_SHORT).show();
					return false;
				}
			}
			...
			if (engineno > 0) {
				if (car.getEngine_no().equals("")) {
					Toast.makeText(WeiZhangMainActivity.this, "输入发动机号不为空", Toast.LENGTH_SHORT).show();
					return false;
				}

				if (car.getEngine_no().length() != engineno) {
					Toast.makeText(WeiZhangMainActivity.this,
							"输入发动机号后" + engineno + "位", Toast.LENGTH_SHORT).show();
					return false;
				}
			} else if (engineno < 0) {
				if (car.getEngine_no().length() == 0) {
					Toast.makeText(WeiZhangMainActivity.this, "输入全部发动机号", Toast.LENGTH_SHORT).show();
					return false;
				}
			}
			...
			if (registno > 0) {
				if (car.getRegister_no().equals("")) {
					Toast.makeText(WeiZhangMainActivity.this, "输入证书编号不为空", Toast.LENGTH_SHORT).show();
					return false;
				}
				...
				if (car.getRegister_no().length() == 0) {
					Toast.makeText(WeiZhangMainActivity.this, "输入全部证书编号", Toast.LENGTH_SHORT).show();
					return false;
				}
			}
			return true;
